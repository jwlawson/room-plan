...
/**
 * Draws a grid of pegs. Touch input used to form room shape. Room is then
 * passed on to DimView.
 * 
 * @author John Lawson
 * 
 */
public class PegView extends View {
	...
	private final VersionedGestureDetector mGestureDetector;
	...
	private class GestureHandler implements VersionedGestureDetector.OnGestureListener {
		...
		public void onDrag(float dx, float dy, float rawX, float rawY) {
			if (mSelectedPeg >= 0 && mSelectedPeg < mPegList.size()) {
				for (int i = 0; i < mCatchList.size(); i++) {
					if (i == mSelectedPeg || i == mLastSelectedPeg)
						continue;

					Circ circ = mCatchList.get(i);
					if (circ.contains(scaledX(rawX), scaledY(rawY))) {
						addLine(mCatchList.get(mSelectedPeg).getX(), mCatchList.get(mSelectedPeg)
								.getY(), circ.getX(), circ.getY());
						if (DEBUG)
							Log.d(TAG, "Dots joined");
